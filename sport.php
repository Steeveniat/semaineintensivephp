...
<head>
	<meta charset="UTF-8">
	<title>Birdie's Arena - Sport</title>
...
<body>
	<main>
		...
		<hr>
		<?php
		$reponse = $pdo->query("SELECT `id_event`, `type`, `titre`, `heure`, `prix`, `description`, `resume`, `ouverture`, `date_event`, `photo`, DATE_FORMAT(date_event,'%e') as jour, DATE_FORMAT(date_event,'%M') as mois, DATE_FORMAT(date_event,'%Y') as annee FROM evenement WHERE type='sport'");

		while ($donnees = $reponse->fetch()){?>
				<div class="details">
					<div class="image"><img src="img-layouts/sport/<?= $donnees['photo']?>" alt="photo"/></div>
